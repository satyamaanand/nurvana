  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      cart = res.data.items.map(item => ({
        ...
        image: item.plant.images[0] || 'images/product-preview.png',
        qty: item.quantity,
  ...
  if (cart.length === 0) {
    listContainer.innerHTML = `
      <div class="cart-empty-state">
        ...
        <a href="buy.html" class="btn btn-secondary btn-sm mt-3" style="width:auto; display:inline-flex;">Browse Plants</a>
      </div>
    `;
...
async function addToCart(plantId, name, price, image, stock = 10) {
  ...
  if (item) {
    ...
    cart.push({
      ...
      image: image || 'images/product-preview.png',
      qty: 1,
      stock
    });
  }
...
function updateNavbarState() {
  ...
  if (!loggedIn) {
    ...
    menuHTML = `
      ...
      <li class="${currentPage === 'buy.html' ? 'active' : ''}"><a href="buy.html" class="nav-link">Plants</a></li>
