...
function renderWishlist(plants) {
  ...
  if (!plants || plants.length === 0) {
    container.innerHTML = `
      <div style="grid-column: span 4; text-align: center; padding: 60px 20px; color: var(--text-muted);">
        ...
        <a href="buy.html" class="btn btn-secondary btn-sm mt-3" style="display:inline-flex; width:auto;">Shop Collection</a>
      </div>
    `;
    return;
  }
  ...
  container.innerHTML = plants.map(plant => {
    const imageUrl = plant.images[0] || 'images/product-preview.png';
